...
def main():
    ...
    print("Iniciando o AI Commit Reporter (Cloud Edition - Groq)...")
    ...
    if not os.getenv("GROQ_API_KEY"):
        print("ERRO: A GROQ_API_KEY não está definida. Verifique o arquivo .env.")
        return

    try:
        # 1. Recupera as informações do Git
        print(
            f"1/3 -> Conectando ao repositório GitHub em "
            f"{os.getenv('GITHUB_URL', 'https://api.github.com')} "
            f"({os.getenv('GITHUB_USER') or os.getenv('GITHUB_ORG')}/"
        ...
        if len(parents) > 1:
            ...
            if pr_match:
                ...
                try:
                    ...
                    print(
                        f"Total de {len(commit_summaries)} commits encontrados na PR #{pr_id}."
                    )
                except Exception as e:
                    print(
                        f"Aviso: Falha ao buscar dados da PR #{pr_id} ({e}). "
                        f"Tentando comparação genérica..."
                    )
                ...
                diff, commit_summaries = git.get_compare_info(parents[0], sha)
                print(
                    f"Total de {len(commit_summaries)} commits encontrados no intervalo."
                )
        else:
        ...
        print(
            "3/3 -> Analisando contexto de código "
            "(isso poderá incluir vários commits se for merge)."
        )
        report, commit_type = processor.process_and_report(
            message, diff, commit_summaries
        )

        # Salva o relatório num arquivo .md dentro da pasta reports
        repo_name = os.getenv("GITHUB_REPO", "repo")
        owner = os.getenv("GITHUB_USER") or os.getenv("GITHUB_ORG")
        ...
        print("Enviando notificação Telegram...")
        dashboard_url = os.getenv("DASHBOARD_URL", "http://localhost:8000")
        repo_sanitized = repo_name.lower().replace(" ", "_").replace("-", "_").replace("/", "_")
        # URL com /sha/ para usar a rota de redirecionamento
...
def save_report(
    ...
    try:
        # O parser do dateutil consegue interpretar quase qualquer formato ISO 8601
        dt_parsed = date_parser.isoparse(date_str)
        ...
        # Converte para o fuso horário alvo usando astimezone
    ...
    # Usamos a data do commit para o nome do arquivo
    data_commit = dt_local.strftime("%Y%m%d_%H%M%S")
    sha_short = sha[:7]
    file_name = os.path.join(
        repo_path,
        f"commit_{repo_sanitized}_{author_sanitized}_{data_commit}_{sha_short}.md"
    )
    ...
    with open(file_name, "w", encoding="utf-8") as file:
        ...
        # Só mostra o diff se ele existir e for valido
        is_valid_diff = diff and any(
            marker in diff for marker in ["--- a/", "diff --git ", "@@ -"]
        )
        ...
        if is_valid_diff and len(diff.strip()) > 50:
            ...
            if not diff:
                ...
                logger.warning(
                    f"Conteúdo não parece ser um Diff válido para o commit {sha[:7]}. "
                    f"Verifique se a API retornou um erro."
                )
